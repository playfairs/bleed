...
async def fetch_channel(channel_id: int):
    async with aiohttp.ClientSession() as session:
        async with session.request(
            ...
            headers={"Authorization": DEFs.TOKEN},
        ) as response:
            data = await response.json()
    return data


async def get_commits(author: str, repository: str):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(
            ...
            headers={
                ...
                "authorization": "token {}".format(DEFs.TOKEN),
            },
        ) as response:
            data = await response.json()
    return data
...
class Commands(Cog):
    ...
    async def git_pull(self):
        ...
        try:
            commits = await get_commits(author, repository)
