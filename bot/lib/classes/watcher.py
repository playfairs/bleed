...
async def auto_commit():
    return
    process = await asyncio.create_subprocess_shell("cd .. ; git init ; git add * ; git commit -m 'auto push' ; git push", stdout = PIPE)
    return True
...
class RebootRunner:
    ...
    def get_path(self):
        if isinstance(self.path, list):
            for path in self.path:
                if not Path(Path.cwd() / path).exists(): return path
        if isinstance(self.path, str):
            if not Path(Path.cwd() / self.path).exists(): return self.path
    ...
    async def load(self, cog_dir: str):
        ...
        try:
            ...
            try:
                last_dot_index = cog_dir.rfind('.')
                if last_dot_index != -1:
                    cog = cog_dir[:last_dot_index]
                else:
                    cog = cog_dir
                try:
