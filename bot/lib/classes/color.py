...
class ColorConverter(commands.Converter):
    async def convert(self, ctx: Context, argument: str) -> Color:
        ...
        try:
            return (await commands.MemberConverter().convert(ctx, argument)).color
        except (commands.CommandError, commands.BadArgument):
            pass

        try:
            return (await commands.RoleConverter().convert(ctx, argument)).color
        except (commands.CommandError, commands.BadArgument):
            pass

        if not (color := get_color(argument)):
            raise commands.CommandError(f"**#{argument}** is an invalid hex code")
